...
class Teacher(nn.Module):
    def __init__(self, n_channels=3, num_classes=2, deep_supervision=False, **kwargs):
        super().__init__()
        ...
        # 第二斜列
        self.conv0_1 = DoubleConv(nb_filter[0] * 1 + nb_filter[1], nb_filter[0], nb_filter[0])
        self.conv1_1 = DoubleConv(nb_filter[1] * 1 + nb_filter[2], nb_filter[1], nb_filter[1])
        self.conv2_1 = DoubleConv(nb_filter[2] * 1 + nb_filter[3], nb_filter[2], nb_filter[2])
        self.conv3_1 = DoubleConv(nb_filter[3] * 1 + nb_filter[4], nb_filter[3], nb_filter[3])
...
if __name__ == "__main__":
    ...
    model = Teacher(deep_supervision=False).to(device)
    print(model)
