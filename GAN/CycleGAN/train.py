...
parser.add_argument('--decay_epoch', type=int, default=39, help='epoch to start linearly decaying the learning rate to 0')
...
if opt.cuda:
    ...
    netD_A.cuda()
    netD_B.cuda()
    # netG_A2B = torch.nn.DataParallel(netG_A2B, device_ids=[0, 1, 2, 3])
    # netG_B2A = torch.nn.DataParallel(netG_B2A, device_ids=[0, 1, 2, 3])
    # netD_A = torch.nn.DataParallel(netD_A, device_ids=[0, 1, 2, 3])
    # netD_B = torch.nn.DataParallel(netD_B, device_ids=[0, 1, 2, 3])
